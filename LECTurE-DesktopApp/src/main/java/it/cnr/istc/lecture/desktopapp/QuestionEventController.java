 ...
 */
public class QuestionEventController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        event.addListener((ObservableValue<? extends QuestionEvent> observable, QuestionEvent oldValue, QuestionEvent newValue) -> {
        ...
        send_answer.setOnAction((ActionEvent event1) -> {
            Context.getContext().answerQuestion(event.get(), group.getToggles().indexOf(group.getSelectedToggle()));
    ...
    public ObjectProperty<QuestionEvent> eventProperty() {
        return event;
    }
}
